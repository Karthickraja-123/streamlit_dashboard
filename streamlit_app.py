...
# CHOROPLETH MAP
    ...
    st.warning("Your dataset does not contain geolocation. Showing demo map.")
    demo = pd.DataFrame({
...
uploaded = st.file_uploader(
    "Upload your MODEL predictions CSV (y_test, y_pred)",
    type=["csv"],
    key="model_uploader"
)
...
if uploaded:
    ...
    if "p_low" in model_df.columns:
        ...
        for i, cls in enumerate(["Low", "Med", "High"]):
            prob_col = f"p_{cls.lower()}"
            fpr, tpr, _ = roc_curve(y_test_bin[:, i], model_df[prob_col])
            roc_auc = auc(fpr, tpr)
